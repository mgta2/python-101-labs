...
'''
# Note: I put a space at the start of above text to get below code to work.
my_text = my_text.lower()
length = len(my_text)
s_index = []
e_index = []
for i in range(1,length):
    if my_text[i].isalpha() and not(my_text[i-1].isalpha()):
        # Then i is starting index of a word
        s_index.append(i)
        # Now find the end index of this word.
        counter = 0
        while True:
            counter += 1
            if not(my_text[i+counter].isalpha()):
                e_index.append(i+counter-1)
                break
sol = ""
for i in range(1,length):
    if i in s_index:
